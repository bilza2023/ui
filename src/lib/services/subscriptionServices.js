
// src/lib/services/subscriptionServices.js
import prisma from '../server/prisma.js';
import { verify, isAdmin } from './loginServices.js';

const DAY_MS = 86_400_000;

function normTcode(t) {
  return (t ?? '').toString().trim();
}

/**
 * Authz helper: given a tcode and a JWT token, decide if user is subscribed.
 * Returns a rich result so callers can show expiry/remaining time in UI.
 * @param {string} tcode
 * @param {string} token
 * @returns {Promise<{allowed:boolean, reason:'ok'|'no-token'|'invalid-token'|'no-subscription'|'expired', userId:string|null, tcode:string, expiresAt:string|null, remainingDays:number|null}>}
 */
export async function isSubscribed(tcode, token) {
  const code = (tcode ?? '').toString().trim();
  if (!token) {
    return { allowed: false, reason: 'no-token', userId: null, tcode: code, expiresAt: null, remainingDays: null };
  }
  if (!code) {
    return { allowed: false, reason: 'no-subscription', userId: null, tcode: code, expiresAt: null, remainingDays: null };
  }

  try {
    const { user } = await verify(token); // throws on invalid/expired

    // Admin bypass (optional but handy)
    if (await isAdmin(token)) {
      return { allowed: true, reason: 'ok', userId: user.id, tcode: code, expiresAt: null, remainingDays: null };
    }

    const now = Date.now();

    // Most recent sub for this user+tcode
    const sub = await prisma.subscription.findFirst({
      where: { user_id: user.id, tcode: code },
      orderBy: { start_date: 'desc' },
      select: { start_date: true, duration: true },
    });

    if (!sub) {
      return { allowed: false, reason: 'no-subscription', userId: user.id, tcode: code, expiresAt: null, remainingDays: null };
    }

    const startMs  = sub.start_date.getTime();
    const expiryMs = startMs + sub.duration * DAY_MS;

    if (now < startMs) {
      // Not active yet; treat as not subscribed for gating
      return { allowed: false, reason: 'no-subscription', userId: user.id, tcode: code, expiresAt: new Date(expiryMs).toISOString(), remainingDays: Math.ceil((expiryMs - now) / DAY_MS) };
    }

    if (now >= expiryMs) {
      return { allowed: false, reason: 'expired', userId: user.id, tcode: code, expiresAt: new Date(expiryMs).toISOString(), remainingDays: 0 };
    }

    const remainingDays = Math.ceil((expiryMs - now) / DAY_MS);
    return { allowed: true, reason: 'ok', userId: user.id, tcode: code, expiresAt: new Date(expiryMs).toISOString(), remainingDays };
  } catch {
    return { allowed: false, reason: 'invalid-token', userId: null, tcode: code, expiresAt: null, remainingDays: null };
  }
}

/**
 * Add a subscription for a user to a given tcode, starting now (or at a provided startDate).
 * @param {string} userId        - Prisma User.id
 * @param {string} tcode         - Subject/code identifier
 * @param {number} durationDays  - Duration in days (integer, >= 1)
 * @param {Object} [opts]
 * @param {Date}   [opts.startDate] - Optional explicit start date (defaults to now, UTC)
 * @returns {Promise<{ id:string, userId:string, tcode:string, start_date:Date, duration:number, expiresAt:string }>}
 */
export async function addSubscription(userId, tcode, durationDays, opts = {}) {
  const uid = (userId ?? '').toString().trim();
  const code = normTcode(tcode);
  const duration = Number.isFinite(durationDays) ? Math.trunc(durationDays) : NaN;
  const startDate = opts.startDate instanceof Date ? opts.startDate : new Date();

  if (!uid) throw new Error('userId is required');
  if (!code) throw new Error('tcode is required');
  if (!Number.isFinite(duration) || duration < 1) throw new Error('duration must be an integer >= 1');

  // Optional: ensure user exists (helps catch typos/foreign key issues)
  const user = await prisma.user.findUnique({ where: { id: uid }, select: { id: true } });
  if (!user) throw new Error('user not found');

  const created = await prisma.subscription.create({
    data: {
      user_id: uid,
      tcode: code,
      start_date: startDate,
      duration,
    },
    select: {
      id: true,
      user_id: true,
      tcode: true,
      start_date: true,
      duration: true,
    },
  });

  const expiryMs = created.start_date.getTime() + created.duration * DAY_MS;
  const expiresAt = new Date(expiryMs).toISOString();

  return {
    id: created.id,
    userId: created.user_id,
    tcode: created.tcode,
    start_date: created.start_date,
    duration: created.duration,
    expiresAt,
  };
}
