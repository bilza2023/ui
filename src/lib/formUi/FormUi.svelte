<script>
    ...
    /**
     * Minimal config-driven form that works with your formKit actions.
     * See previous message for the config schema summary.
     */
    ...
    // --- Internal state ---
    let formResult = null;   // last server result { ok, message, values, ... }
    let values = {};         // sticky values
  
    // Helper for stable input ids
    ...
    // Build initial values from config.initial and item.value
    function initValuesFromConfig() {
      const init = { ...(config?.initial ?? {}) };
      const items = Array.isArray(config?.items) ? config.items : [];
      for (const item of items) {
        if (!item || item.type === 'note' || (item.type === 'hidden' && !item.name)) continue;
        const key = item.name;
        if (!key) continue;
        if (init[key] === undefined) {
          init[key] = item.value !== undefined ? item.value : defaultFor(item);
        }
      }
      return init;
    }
    ...
    // Re-init values whenever config changes
    ...
    function onInput(name, cast, ev) {
      const raw = ev?.target?.value;
      const v = cast ? cast(raw) : raw;
      values = { ...values, [name]: v };
    }
  
    function onCheck(name, ev) {
      const v = !!ev?.target?.checked;
      values = { ...values, [name]: v };
    }
  
    function disabledSubmit() {
      const rule = config?.submit?.disabledWhen;
      return isFn(rule) ? !!rule(values) : false;
    }
  
    // Enhance hook → cooperate with formKit action result shape
    function handleEnhance() {
      return ({ result }) => {
        if (result.type === 'failure') {
          formResult = result.data;                       // { ok:false, message, values, errors? }
          if (formResult?.values) values = { ...values, ...formResult.values };
          dispatch('failure', formResult);
          return;
        }
        if (result.type === 'success') {
          formResult = result.data;                       // { ok:true, saved, message?, ... }
            ...
            <div class="f-row {item?.type}">
              ...
              {#if item.type === 'textarea'}
                <textarea
                  ...
                  on:input={(e)=>onInput(item.name, null, e)}
                >{values[item.name] ?? ''}</textarea>
              ...
              {:else if item.type === 'select'}
                <select
                  ...
                  on:change={(e)=>onInput(item.name, null, e)}
                >
              ...
              {:else if item.type === 'number'}
                <input
                  ...
                  disabled={itemDisabled(item)}
                  value={values[item.name] ?? ''}
                  on:input={(e)=>onInput(item.name, (x)=>x, e)}
                />
              ...
              {:else if item.type === 'checkbox'}
                <div class="f-check">
                  <input
                    ...
                    on:change={(e)=>onCheck(item.name, e)}
                    disabled={itemDisabled(item)}
                  />
                  <span class="check-label">{item.placeholder}</span>
                </div>
