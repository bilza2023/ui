<script>
  ...
  function hrefNotes(t) {
    return `/subject?tcode=${encodeURIComponent(t.tcodeName)}&type=note`;
  }
...
<div class="wrap">
  ...
  {:else}
    <div class="grid">
      {#each filtered as t}
        <a class="card" href={hrefNotes(t)} title={`Browse notes in ${t.tcodeName}`}>
          <div class="thumb">
            <img src={t.image || FALLBACK_IMG} alt={t.tcodeName} loading="lazy" />
            <div class="badge note">Notes</div>
          </div>
          <div class="body">
...
<style>
  ...
  .badge {
    ...
    border-radius:999px; font-size:12px; padding:2px 8px; border:1px solid rgba(255,255,255,0.12);
  }
  .badge.note { background:#6c430b; color:#ffe3c4; }
