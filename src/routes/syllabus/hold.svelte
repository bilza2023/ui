<script>
  ...
  // Local state (mirrors data.*; CSS/layout untouched)
  let tcode = null;
  let chapters = [];
  let chapter = null;
  let exercises = [];
  ...
  $: counts = data?.counts ?? {};
  $: items = data?.items ?? [];
  $: typeFilter = data?.typeFilter ?? null;

  // description from tcodeList (supports either .tcode or .tcodeName)
  $: tcodeDesc =
    (data?.tcodeList ?? []).find((t) => (t.tcode ?? t.tcodeName) === tcode)
      ?.description ?? "";

  // URL is the state (unchanged)
  function go(params) {
...
{:else}
  ...
  <div class="topbar one">
    ...
    <div class="filters">
      <span>Filter:</span>
      <button class:active={!typeFilter} on:click={() => setTypeFilter(null)}
        >All</button
      >
      <button
        class:active={typeFilter === "deck"}
        on:click={() => setTypeFilter("deck")}>Decks</button
      >
      <button
        class:active={typeFilter === "note"}
        on:click={() => setTypeFilter("note")}>Notes</button
      >
    </div>
  </div>
...
<style>
  ...
  .topbar {
    ...
    border-bottom: 1px solid
      color-mix(in oklab, var(--accentColor) 45%, var(--borderColor));
    position: sticky;
    top: 0;
    z-index: 10;
    color: var(--primaryText);
  ...
  .exbar-wrap {
    ...
    gap: 10px;
    border-bottom: 1px solid
      color-mix(in oklab, var(--accentColor) 35%, var(--borderColor));
  }
  ...
  @media (max-width: 700px) {
    ...
    .exbar-wrap {
      top: 98px;
    } /* cushion if header wraps on mobile */
  }
</style>
