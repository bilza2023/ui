...
<script>
  ...
  >
    {#if placeholder}
      <option value="" disabled selected={!selectedStr}>{placeholder}</option>
    {/if}
    {#each data.surat as s}
      <option value={s.surah}>{s.surah}. {s.name}</option>
    {/each}
  </select>
  ...
  <style>
    ...
    .surah-dd:focus {
      ...
      box-shadow: 0 0 0 3px color-mix(in oklab, var(--accent, #4f46e5) 25%, transparent);
    }
  </style>
