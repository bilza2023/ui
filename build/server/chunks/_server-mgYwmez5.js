import { j as json, r as redirect } from './index-BIDRY2MQ.js';
import { d as deleteByFilename } from './questionServices-CHZd5-Cj.js';
...
async function POST({ request }) {
  ...
  if (!filename) {
    ...
    return wantsJson(request) ? json({ success: false, error: msg }, { status: 400 }) : (() => {
      throw redirect(303, `/admin?error=${encodeURIComponent(msg)}`);
    })();
  }
  try {
    await deleteByFilename(filename);
    return wantsJson(request) ? json({ success: true, deleted: filename }) : (() => {
    ...
    return wantsJson(request) ? json({ success: false, error: msg }, { status: notFound ? 404 : 500 }) : (() => {
      throw redirect(303, `/admin?error=${encodeURIComponent(msg)}`);
    })();
  }
}
...
export { POST };
//# sourceMappingURL=_server-mgYwmez5.js.map
