...
const TcodeCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="flex flex-wrap gap-10 justify-center">${each(tcodes, (t) => {
    return `<a${add_attribute("href", `/syllabus?tcode=${t.tcodeName}`, 0)}><div class="block flex-none w-64 mx-auto rounded-xl shadow-lg overflow-hidden hover:ring-2 ring-yellow-600 transition-all bg-white"><img${add_attribute("src", t.image, 0)}${add_attribute("alt", t.tcodeName, 0)} class="w-full h-48 object-cover"> <div class="p-4"><h2 class="text-lg font-bold text-gray-800">${escape(t.tcodeName)}</h2> <p class="text-sm text-gray-600 mt-1">${escape(t.tcodeName)}</p> </div></div> </a>`;
  })}</div>`;
});
...
//# sourceMappingURL=_page.svelte-DgKnDc2R.js.map
